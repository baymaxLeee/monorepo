...
第一段在 `apps/backend/services/chat/src/agent/agents/tool-loop.ts`。项目把模型循环交给 SDK，只在 `prepareStep` 里读取已经发生的 Step，并用 Artifact 状态机强制下一步：
...
本地 Chat Service 的真实 HTTP Run 会在 `ToolLoopAgent` 创建前捕获 `assembleInstructions()` 的最终返回值。完整快照位于 `apps/backend/services/chat/src/agent/context/system-prompt.xml`：约 34KB，前半部分是 Runtime 实际消费的英文 Instructions，后半部分是逐段中文翻译；中文部分只供分享阅读，不会注入模型。
...
| 分享主题 | 项目实现 |
|---|---|
| Run orchestration / persistence / continuation | `apps/backend/services/chat/src/agent/runs/run.ts` |
| ToolLoopAgent / prepareStep / toolsContext | `apps/backend/services/chat/src/agent/agents/tool-loop.ts` |
| Instructions assembly | `apps/backend/services/chat/src/agent/context/instructions/` |
| 真实双语 Instructions | `apps/backend/services/chat/src/agent/context/system-prompt.xml` |
| Context budget / file transform / ModelMessage projection | `apps/backend/services/chat/src/agent/context/projector.ts` |
| Tool Catalog / Manifest policy | `apps/backend/services/chat/src/agent/tools/catalog.ts`、`manifest.ts` |
| Search / Artifact Tool Schema | `apps/backend/services/chat/src/agent/tools/builtins/search.ts`、`artifacts.ts` |
| HTML quality-gate state machine | `apps/backend/services/chat/src/agent/agents/artifact-verification.ts` |
| Durable HTML Workflow | `apps/backend/services/executor/workflows/html-artifact.ts` |
| Typed Outline / Block generation prompt | `apps/backend/services/executor/src/artifacts/generator.ts` |
| Grid/Flex/Chart Template contract | `apps/backend/services/executor/src/artifacts/compiler.ts`、`template.ts` |
| Static validator / model reviewer | `apps/backend/services/executor/src/artifacts/validator.ts`、`reviewer.ts` |
