...
function applyEvent(
  ...
  if (outcome.kind === "failed") {
    const expectedTool = state.current?.phase === "verify" ? "validate_html" : "edit_file";
  ...
  if (toolName !== "validate_html" || !state.current) return state;
  if (output.file_id !== state.current.documentId) {
    return { ...state, current: { ...state.current, phase: "failed", failure: "validate_html did not complete for the expected revision" } };
  }
...
export function artifactVerificationDirective(state: ArtifactVerificationState): ArtifactVerificationDirective | null {
  ...
  if (item.phase === "verify") {
    return {
      ...
      instruction: `Quality gate: call validate_html now with file_id="${item.documentId}". Do not answer the user before validation completes.`,
    };
  }
