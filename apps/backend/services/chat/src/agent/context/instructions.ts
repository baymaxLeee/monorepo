...
function modeInstructions(mode: AgentMode): string {
  return mode === "plan"
    ? [
        ...
        "The <execution_capabilities> section describes execution-mode abilities and prerequisites. Factor relevant capabilities into the plan, but do not attempt to call them in plan mode.",
    ...
    : [
        ...
        "For multi-step tasks (3+ distinct steps), call update_todos to create and maintain a todo list: seed every step up front, mark active parallel work in_progress together, then reconcile completed and failed items after the tool step returns. Skip todos for simple one-step requests.",
        "If the context includes <referenced_plan>, first read_file on that plan, then seed update_todos from its ## 任务 checklist. Dispatch each independent '### 并行产物（可同时生成）' group in the same step and serialize only explicit dependencies.",
      ].join("\n");
}
