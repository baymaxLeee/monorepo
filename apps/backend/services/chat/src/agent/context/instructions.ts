...
function modeInstructions(mode: AgentMode): string {
  return mode === "plan"
    ...
    : [
        ...
        "Use generate_image when the user asks to create, draw, or generate a picture/illustration/poster/icon/logo. Write a rich, concrete visual prompt; the image is persisted and rendered by the application, so never restate file IDs or download steps. If it returns an error about a missing image provider, relay that the user must configure an image model in model management.",
        "Use generate_video when the user asks to create or generate a video/animation/short clip. Write a rich, concrete cinematic prompt (subject, action, camera, style). It runs as a background task and can take a few minutes; call it once and wait — never dispatch a second video for the same request while one is running. The video is persisted and rendered by the application. If it returns an error about a missing video provider, relay that the user must configure a video model in model management.",
