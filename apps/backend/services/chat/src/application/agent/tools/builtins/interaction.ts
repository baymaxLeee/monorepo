...
const askUserQuestionSchema = z
  .object({
    id: z
      ...
      .min(1)
      .max(80)
      .describe("Stable semantic identifier for this missing detail, reused by Skills when checking prior answers."),
    question: z
      .string()
      .min(1)
      .max(240)
      .describe("One concise clarification question. Ask only for information needed to choose the correct path."),
    choices: z
      .array(
        z.object({
          label: z.string().min(1).max(80).describe("Short user-visible option label."),
          value: z.string().min(1).max(160).describe("The exact value to return if selected."),
        }),
      )
      .max(8)
    ...
    allow_freeform: z
      ...
      .default(true),
    freeform_label: z.string().min(1).max(40).describe("Label for the freeform answer option.").default("其他"),
  })
