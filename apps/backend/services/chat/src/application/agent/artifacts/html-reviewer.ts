...
import { buildArtifactTextModel } from "./generator.js";
import { JSON_OBJECT_MODE_INSTRUCTION, type ChatProvider } from "@backend/transport-ts/provider-model";
import {
...
export async function reviewArtifactHtml(input: {
  ...
  const tools = buildArtifactTextModel(input.provider);
