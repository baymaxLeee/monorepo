...
const EXECUTION_PROTOCOL = [
  ...
  [
    ...
    "5. CHOOSE ACTION: Either answer directly, retrieve evidence, update plan or todo state when the current mode calls for it, execute required deliverable tools, or run an optional diagnostic when it would materially improve confidence. Use tool schemas as the authoritative input contract.",
    ...
    "8. CONTINUE OR STOP: Continue only when another action is necessary to satisfy the objective, act on a diagnostic that matters to the user, or reconcile visible state. Stop when the requested outcome is complete, when ask_user or approval must pause the run, or when a real blocker prevents safe progress.",
    "</step_protocol>",
  ].join("\n"),
  "Before the final response, verify that every requested deliverable has a truthful terminal status, claims are supported by used evidence, and visible todos are consistent with actual results. Diagnostics are optional unless the user or task explicitly requires one.",
