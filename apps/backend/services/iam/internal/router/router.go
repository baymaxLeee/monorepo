...
func (rt *Router) applyMembership(w http.ResponseWriter, r *http.Request) {
	...
	err := rt.org.Apply(r.Context(), chi.URLParam(r, "orgID"), claims.Subject, service.AuditMetaFromHTTP(r, claims.Subject))
	switch {
	case errors.Is(err, service.ErrOrgNotFound):
...
func (rt *Router) switchActiveOrg(w http.ResponseWriter, r *http.Request) {
	...
	var req schema.SwitchOrgRequest
	if !decodeJSON(w, r, &req) {
		return
	}
	response, refreshToken, refreshExpiresAt, err := rt.auth.SwitchActiveOrg(
		r.Context(), cookie.Value, req.OrgID, claims.Subject,
		service.RequestMetaFromHTTP(r), service.AuditMetaFromHTTP(r, claims.Subject),
	)
	switch {
	case errors.Is(err, service.ErrInvalidOrg):
		writeProblem(w, http.StatusBadRequest, "invalid_org", "orgId is required")
		return
	case errors.Is(err, service.ErrNotActiveMember):
...
func (rt *Router) createOrg(w http.ResponseWriter, r *http.Request) {
	...
	view, err := rt.org.Create(r.Context(), req, service.AuditMetaFromHTTP(r, claims.Subject))
	switch {
	case errors.Is(err, service.ErrInvalidOrg):
...
func (rt *Router) createOrgAdmin(w http.ResponseWriter, r *http.Request) {
	...
	var req schema.CreateOrgAdminRequest
	if !decodeJSON(w, r, &req) {
		return
	}
	view, err := rt.org.CreateOrgAdmin(r.Context(), chi.URLParam(r, "orgID"), req, service.AuditMetaFromHTTP(r, claims.Subject))
	switch {
	case errors.Is(err, service.ErrOrgNotFound):
		writeProblem(w, http.StatusNotFound, "organization_not_found", "organization does not exist")
		return
...
func (rt *Router) transferOwner(w http.ResponseWriter, r *http.Request) {
	...
	var req schema.TransferOwnerRequest
	if !decodeJSON(w, r, &req) {
		return
	}
	err := rt.org.TransferOwner(r.Context(), chi.URLParam(r, "orgID"), req.NewOwnerUserID, service.AuditMetaFromHTTP(r, claims.Subject))
	switch {
	case errors.Is(err, service.ErrInvalidOrg):
	...
	case errors.Is(err, service.ErrNotFound):
		writeProblem(w, http.StatusNotFound, "organization_not_found", "organization does not exist")
		return
	case errors.Is(err, service.ErrInvariant):
...
func (rt *Router) approveMember(w http.ResponseWriter, r *http.Request) {
	...
	err := rt.org.Approve(r.Context(), orgID, chi.URLParam(r, "userID"), claims.Subject, service.AuditMetaFromHTTP(r, claims.Subject))
...
func (rt *Router) rejectMember(w http.ResponseWriter, r *http.Request) {
	...
	var req schema.RejectMemberRequest
	if !decodeJSON(w, r, &req) {
		return
	}
	err := rt.org.Reject(r.Context(), orgID, chi.URLParam(r, "userID"), claims.Subject, req.Reason, service.AuditMetaFromHTTP(r, claims.Subject))
	rt.writeMembershipResult(w, err)
}
...
func (rt *Router) setMemberRole(w http.ResponseWriter, r *http.Request) {
	...
	var req schema.SetMemberRoleRequest
	if !decodeJSON(w, r, &req) {
		return
	}
	err := rt.org.SetMemberRole(r.Context(), orgID, chi.URLParam(r, "userID"), req.Role, service.AuditMetaFromHTTP(r, claims.Subject))
...
func (rt *Router) assignUserRole(w http.ResponseWriter, r *http.Request) {
	...
	if err := rt.roles.Assign(r.Context(), chi.URLParam(r, "userID"), req.RoleID, service.AuditMetaFromHTTP(r, claims.Subject)); err != nil {
...
func (rt *Router) removeUserRole(w http.ResponseWriter, r *http.Request) {
	...
	err := rt.roles.Remove(r.Context(), chi.URLParam(r, "userID"), chi.URLParam(r, "roleID"), service.AuditMetaFromHTTP(r, claims.Subject))
	switch {
