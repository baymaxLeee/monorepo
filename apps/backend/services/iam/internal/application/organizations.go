...
func NewOrgService(store *repositories.Store, cfg config.Config) *OrgService {
...
func (s *OrgService) ListPublic(ctx context.Context) ([]contracts.OrgSummary, error) {
	orgs, err := s.store.ListOrganizations(ctx)
	if err != nil {
		return nil, err
	}
	out := make([]contracts.OrgSummary, 0, len(orgs))
	for _, org := range orgs {
		out = append(out, contracts.OrgSummary{ID: org.ID, Name: org.Name})
	}
	return out, nil
}

func (s *OrgService) ListForAdmin(ctx context.Context) ([]contracts.OrgAdminView, error) {
	rows, err := s.store.ListOrganizationsForAdmin(ctx)
	if err != nil {
		return nil, err
	}
	out := make([]contracts.OrgAdminView, 0, len(rows))
	for _, row := range rows {
		out = append(out, contracts.OrgAdminView{
...
func (s *OrgService) Create(ctx context.Context, req contracts.CreateOrgRequest, meta AuditMeta) (contracts.OrgAdminView, error) {
	...
	if hasExisting == hasInline {
		return contracts.OrgAdminView{}, ErrInvalidOrg // exactly one owner source
	}
	now := time.Now().UTC()
	...
	if hasExisting {
		...
		if _, err := s.store.UserByID(ctx, ownerID); err != nil {
			return contracts.OrgAdminView{}, ErrOwnerNotFound
		}
		...
		if err != nil {
			return contracts.OrgAdminView{}, err
		}
...
func (s *OrgService) CreateOrgAdmin(ctx context.Context, orgID string, req contracts.CreateOrgAdminRequest, meta AuditMeta) (contracts.OrgMemberView, error) {
	...
	if err != nil {
		return contracts.OrgMemberView{}, err
	}
	...
	if err := mutateWithAudit(ctx, s.store, auditEntry{
		...
		return contracts.OrgMemberView{}, ErrConflict
	}
	return contracts.OrgMemberView{
		UserID:      user.ID,
...
func (s *OrgService) ListMembers(ctx context.Context, orgID, status string) ([]contracts.OrgMemberView, error) {
	rows, err := s.store.ListOrgMembers(ctx, orgID, strings.TrimSpace(status))
	if err != nil {
		return nil, err
	}
	out := make([]contracts.OrgMemberView, 0, len(rows))
	for _, row := range rows {
...
func orgMemberView(row repositories.OrgMemberRow) contracts.OrgMemberView {
	view := contracts.OrgMemberView{
		UserID:          row.UserID,
		Account:         row.Account,
