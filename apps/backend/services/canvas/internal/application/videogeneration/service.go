	...
	err := s.transactions.WithinTransaction(ctx, func(txCtx context.Context) error {
		...
		if terminalStatus(poll.TerminalStatus) {
			...
			if poll.TerminalStatus == domaintask.StatusSucceeded {
				...
				taskRunStatus = domaintask.StatusWaitingSubtasks
				finishedAt = nil
			}
			...
			updated, updateErr = s.runs.UpdateTaskRun(txCtx, run, TaskRunUpdate{
				Status: taskRunStatus, ErrorMessage: poll.ErrorMessage, FinishedAt: finishedAt,
			}, now)
			if updateErr != nil || !updated {
				return updateErr
			}
...
func (s *Service) createFirstLastFrameTask(ctx context.Context, target firstLastFrameTarget, parent domaintask.TaskRun, now time.Time) error {
	taskRunID, err := s.ids.NewID()
	if err != nil {
		return err
	}
	...
	if parent.ID != target.GenerationTaskRunID || parent.TenantID != target.TenantID || !sameWorkspaceID(parent.WorkspaceID, target.WorkspaceID) {
		return applicationtask.ErrInvalidTaskHierarchy
	}
	parentTaskID := parent.ID
	run := domaintask.TaskRun{
		ID: taskRunID, TenantID: target.TenantID, WorkspaceID: target.WorkspaceID,
		RootTaskID: parent.EffectiveRootTaskID(), ParentTaskID: &parentTaskID,
