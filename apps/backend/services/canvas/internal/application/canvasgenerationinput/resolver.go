...
type ResourceResolver interface {
	ResolveCurrentResourceAsset(context.Context, applicationcanvas.Scope, string, string) (applicationcanvas.CanvasResourceAssetReference, error)
	ResolvePrimaryResourceAsset(context.Context, applicationcanvas.Scope, string, string) (applicationcanvas.CanvasResourceAssetReference, error)
}
...
func (resolver *Resolver) ResolveMentions(ctx context.Context, scope applicationcanvas.Scope, projectID string, target domaincanvas.CanvasNode, nodes []domaincanvas.CanvasNode, allowed AllowedModalities) (Result, error) {
	mentionIDs, err := domaincanvas.AssetMentionIDs(target.Prompt)
	if err != nil {
		return Result{}, err
	}
...
func (resolver *Resolver) ResolveFrames(ctx context.Context, scope applicationcanvas.Scope, projectID string, target domaincanvas.CanvasNode, nodes []domaincanvas.CanvasNode) (Result, error) {
	byID := make(map[string]domaincanvas.CanvasNode, len(nodes))
	for _, node := range nodes {
		byID[node.ID] = node
	}
...
func (resolver *Resolver) resolveNode(ctx context.Context, scope applicationcanvas.Scope, projectID string, source domaincanvas.CanvasNode, role domaingenerationinput.Role) (domaingenerationinput.Input, error) {
	if source.ReferenceStatus == domaincanvas.ReferenceStatusDeleted {
