...
type PresetSkillState string

const (
	PresetSkillActive  PresetSkillState = "active"
	PresetSkillRetired PresetSkillState = "retired"
)

type PresetSkill struct {
	Key                string
	AssetCenterSkillID string
	Name               string
	DefaultBound       bool
	State              PresetSkillState
}

type PresetSkillStore interface {
	UpsertPresetSkill(context.Context, PresetSkill) error
}
