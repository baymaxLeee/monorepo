...
import (
	...
	applicationcanvasarchive "github.com/example/monorepo/canvas/internal/server/application/canvasarchive"
...
func (h *CanvasArchiveHandler) StartProjectCanvasVideoArchiveExport(ctx context.Context, request *contractcanvas.StartProjectCanvasVideoArchiveExportRequest) (*contractcanvas.StartProjectCanvasVideoArchiveExportResponse, error) {
	...
	return &contractcanvas.StartProjectCanvasVideoArchiveExportResponse{Export: archiveExportDTO(item, h.now())}, nil
}

func (h *CanvasArchiveHandler) GetProjectCanvasVideoArchiveExport(ctx context.Context, request *contractcanvas.GetProjectCanvasVideoArchiveExportRequest) (*contractcanvas.GetProjectCanvasVideoArchiveExportResponse, error) {
	...
	return &contractcanvas.GetProjectCanvasVideoArchiveExportResponse{Export: archiveExportDTO(item, h.now())}, nil
}

func (h *CanvasArchiveHandler) BatchGetProjectCanvasVideoArchiveExports(ctx context.Context, request *contractcanvas.BatchGetProjectCanvasVideoArchiveExportsRequest) (*contractcanvas.BatchGetProjectCanvasVideoArchiveExportsResponse, error) {
	...
	now := h.now()
	response := make([]*contractcanvas.ProjectCanvasVideoArchiveExport, 0, len(items))
	for index := range items {
		response = append(response, archiveExportDTO(items[index], now))
	}
	return &contractcanvas.BatchGetProjectCanvasVideoArchiveExportsResponse{Items: response}, nil
}

func (h *CanvasArchiveHandler) ListProjectCanvasVideoArchiveExports(ctx context.Context, request *contractcanvas.ListProjectCanvasVideoArchiveExportsRequest) (*contractcanvas.ListProjectCanvasVideoArchiveExportsResponse, error) {
	...
	now := h.now()
	responseItems := make([]*contractcanvas.ProjectCanvasVideoArchiveExport, 0, len(items))
	for index := range items {
		responseItems = append(responseItems, archiveExportDTO(items[index], now))
	}
	return &contractcanvas.ListProjectCanvasVideoArchiveExportsResponse{
		Items: responseItems,
		Page:  pageOutput(request.Page.PageSize, request.Page.PageNum, total),
	}, nil
}

func (h *CanvasArchiveHandler) CancelProjectCanvasVideoArchiveExport(ctx context.Context, request *contractcanvas.CancelProjectCanvasVideoArchiveExportRequest) (*contractbase.Empty, error) {
...
func archiveSortDirection(sort *contractcanvas.ProjectCanvasVideoArchiveExportSort) (applicationcanvasarchive.SortDirection, error) {
	if sort == nil {
		return applicationcanvasarchive.SortUnspecified, nil
	}
	if sort.IsSetField() &&
		sort.GetField() != contractcanvas.ProjectCanvasVideoArchiveExportSortField_CREATED_AT {
...
func archiveExportDTO(item applicationcanvasarchive.Export, now time.Time) *contractcanvas.ProjectCanvasVideoArchiveExport {
	dto := &contractcanvas.ProjectCanvasVideoArchiveExport{
	...
	if item.RetentionGuaranteedUntil != nil {
		value := contractcommon.Timestamp(timestamp(*item.RetentionGuaranteedUntil))
		dto.RetentionGuaranteedUntil = &value
	}
	if item.StartedAt != nil {
		value := contractcommon.Timestamp(timestamp(*item.StartedAt))
		dto.StartedAt = &value
	}
	if item.FinishedAt != nil {
		value := contractcommon.Timestamp(timestamp(*item.FinishedAt))
...
func archiveStatus(status domaintask.Status) contractcanvas.CanvasVideoArchiveExportStatus {
	switch status {
	case domaintask.StatusQueued:
		return contractcanvas.CanvasVideoArchiveExportStatus_QUEUED
	case domaintask.StatusRunning:
		return contractcanvas.CanvasVideoArchiveExportStatus_RUNNING
	case domaintask.StatusSucceeded:
		return contractcanvas.CanvasVideoArchiveExportStatus_SUCCEEDED
	case domaintask.StatusFailed:
		return contractcanvas.CanvasVideoArchiveExportStatus_FAILED
	case domaintask.StatusCancelled:
		return contractcanvas.CanvasVideoArchiveExportStatus_CANCELLED
	default:
		return 0
	}
}
