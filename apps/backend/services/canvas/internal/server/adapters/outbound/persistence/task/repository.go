...
import (
	...
	app "github.com/example/monorepo/canvas/internal/server/application/task"
	domain "github.com/example/monorepo/canvas/internal/server/domain/task"
)
...
func (r *Repository) ClaimDuePollSchedules(ctx context.Context, now, leaseUntil time.Time, limit int) ([]domain.PollSchedule, error) {
	return r.claimDuePollSchedules(ctx, now, leaseUntil, limit, false)
}

func (r *Repository) ClaimDueImageGenerationPollSchedules(ctx context.Context, now, leaseUntil time.Time, limit int) ([]domain.PollSchedule, error) {
	return r.claimDuePollSchedules(ctx, now, leaseUntil, limit, true)
}

func (r *Repository) claimDuePollSchedules(ctx context.Context, now, leaseUntil time.Time, limit int, imageGeneration bool) ([]domain.PollSchedule, error) {
	if limit <= 0 {
		return nil, nil
	}
	db := r.dbFor(ctx)
	...
	).Where("NOT EXISTS (SELECT 1 FROM task_runs WHERE task_runs.id = poll_schedules.task_run_id AND task_runs.hidden_at IS NOT NULL)")
	if imageGeneration {
		query = query.Where("EXISTS (SELECT 1 FROM task_runs WHERE task_runs.id = poll_schedules.task_run_id AND task_runs.run_type = ?)", domain.RunTypeImageGeneration)
	} else {
		query = query.Where("NOT EXISTS (SELECT 1 FROM task_runs WHERE task_runs.id = poll_schedules.task_run_id AND task_runs.run_type = ?)", domain.RunTypeImageGeneration)
	}
	...
	for index := range candidates {
		...
		).Where("NOT EXISTS (SELECT 1 FROM task_runs WHERE task_runs.id = poll_schedules.task_run_id AND task_runs.hidden_at IS NOT NULL)").Updates(map[string]any{
...
func (r *Repository) update(ctx context.Context, taskRunID string, expectedVersion int64, updates map[string]any) (bool, error) {
	...
	result := r.dbFor(ctx).Model(&taskRunRow{}).Where(
		...
		[]string{domain.StatusQueued, domain.StatusRunning},
	).Updates(updates)
	if result.Error != nil {
		return false, result.Error
	}
