...
import (
	...
	applicationfirstlastframe "github.com/example/monorepo/canvas/internal/server/application/firstlastframe"
...
func (r *Repository) BatchGetFirstLastFrameTaskRunIDs(
	ctx context.Context,
	scope app.Scope,
	...
	var rows []struct {
		...
		FirstLastFrameTaskRunID *persistenceid.UUID
	}
	query := scopeQuery(r.dbFor(ctx).Model(&canvasnodeVideoGenerationRow{}), scope)
	if err := query.
		Select("task_run_id", "first_last_frame_task_run_id").
		Where("task_run_id IN ? AND hidden_at IS NULL AND first_last_frame_task_run_id IS NOT NULL", ids).
		Find(&rows).Error; err != nil {
		return nil, err
	}
	for _, row := range rows {
		result[row.TaskRunID.String()] = row.FirstLastFrameTaskRunID.String()
	}
	return result, nil
}
...
func (r *Repository) ListFirstLastFrameCandidates(
	ctx context.Context,
	scope app.Scope,
	taskRunIDs []string,
) ([]domainvideo.Generation, error) {
	ids := make([]persistenceid.UUID, 0, len(taskRunIDs))
	seen := make(map[persistenceid.UUID]struct{}, len(taskRunIDs))
	for _, raw := range taskRunIDs {
	...
	if len(ids) == 0 {
		return []domainvideo.Generation{}, nil
	}
	var rows []canvasnodeVideoGenerationRow
	// The locking join makes "currently selected" and task attachment one transaction boundary:
	// a concurrent history selection must wait until registration commits or rolls back.
	query := r.dbFor(ctx).
		Table("canvas_node_generations AS generations").
		Select("generations.*").
		Joins(`JOIN canvas_nodes
			ON canvas_nodes.id = generations.node_id
			AND canvas_nodes.project_id = generations.project_id
			AND canvas_nodes.canvas_id = generations.canvas_id
			AND canvas_nodes.selected_output_id = generations.task_run_id`).
		Where("canvas_nodes.deleted_at = ?", 0)
	query = qualifiedScopeQuery(query, "generations", scope)
	query = qualifiedScopeQuery(query, "canvas_nodes", scope)
	err := query.
		Clauses(clause.Locking{Strength: "UPDATE"}).
		Where("generations.task_run_id IN ?", ids).
		Where("generations.hidden_at IS NULL").
		Where("generations.status = ?", string(domaintask.StatusSucceeded)).
		Where("generations.asset_id IS NOT NULL").
		Where("generations.first_last_frame_task_run_id IS NULL").
		Where("generations.first_frame_asset_id IS NULL OR generations.last_frame_asset_id IS NULL").
		Order("generations.task_run_id ASC").
		Find(&rows).Error
	if err != nil {
		return nil, err
	}
