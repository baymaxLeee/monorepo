...
import (
	...
	applicationcanvasimagegeneration "github.com/example/monorepo/canvas/internal/server/application/canvasimagegeneration"
	applicationtextgeneration "github.com/example/monorepo/canvas/internal/server/application/canvastextgeneration"
...
type VideoFrameTaskBatchReader interface {
	...
	BatchGetFirstLastFrameTaskRunIDs(context.Context, applicationcanvas.Scope, []string) (map[string]string, error)
...
type VideoFrameStateFailureReporter interface {
	ReportVideoFrameStateFailure(context.Context, string, string, error)
}
...
type Service struct {
	...
	frameFailures   VideoFrameStateFailureReporter
}
...
func WithVideoFrameStateFailureReporter(reporter VideoFrameStateFailureReporter) ServiceOption {
	return func(service *Service) {
		if reporter != nil {
			service.frameFailures = reporter
		}
	}
}
...
func NewService(nodes NodeReader, images ImageEngine, videos *applicationvideogeneration.Service, texts *applicationtextgeneration.Service, assets AssetPreviewer, options ...ServiceOption) *Service {
	service := &Service{
		...
		frameFailures: noopVideoFrameStateFailureReporter{},
	}
...
func (s *Service) BatchGetStates(ctx context.Context, scope applicationcanvas.Scope, projectID, canvasID string, targets []GenerationTarget) ([]GenerationState, error) {
	...
	if err = s.applyVideoFrameCompletion(ctx, scope, runs); err != nil {
		return nil, classify(err)
	}
...
func (s *Service) applyVideoFrameCompletion(
	ctx context.Context,
	scope applicationcanvas.Scope,
	runs []domaintask.TaskRun,
) error {
	videoTaskRunIDs := make([]string, 0, len(runs))
	for _, run := range runs {
		if run.RunType == domaintask.RunTypeCanvasNodeVideoGeneration && run.Status == domaintask.StatusSucceeded {
			videoTaskRunIDs = append(videoTaskRunIDs, run.ID)
		}
	}
	if len(videoTaskRunIDs) == 0 {
	...
	frameTaskIDsByVideo, err := s.videoFrameTasks.BatchGetFirstLastFrameTaskRunIDs(ctx, scope, videoTaskRunIDs)
	if err != nil {
		return err
	}
	frameTaskRunIDs := make([]string, 0, len(frameTaskIDsByVideo))
	for _, videoTaskRunID := range videoTaskRunIDs {
		if frameTaskRunID := frameTaskIDsByVideo[videoTaskRunID]; frameTaskRunID != "" {
			frameTaskRunIDs = append(frameTaskRunIDs, frameTaskRunID)
		}
	}
	if len(frameTaskRunIDs) == 0 {
		return nil
	}
	frameRuns, err := s.generationRuns.BatchGetScopedTaskRuns(ctx, applicationtask.Scope{
		TenantID: scope.TenantID, WorkspaceID: scope.WorkspaceID,
	}, frameTaskRunIDs)
	if err != nil {
		return err
	}
	frameRunsByID := make(map[string]domaintask.TaskRun, len(frameRuns))
	for _, run := range frameRuns {
		frameRunsByID[run.ID] = run
	}
	for index := range runs {
		frameTaskRunID := frameTaskIDsByVideo[runs[index].ID]
		if frameTaskRunID == "" {
			continue
		}
		frameRun, ok := frameRunsByID[frameTaskRunID]
		if !ok || !frameRun.IsInternal || frameRun.RunType != domaintask.RunTypeCanvasNodeVideoFirstLastFrameExtraction ||
			frameRun.SubjectType != domaintask.SubjectTypeCanvasNode || frameRun.SubjectID != runs[index].SubjectID {
			reportCtx := logcontext.WithBusiness(ctx, logcontext.Business{
				TenantID: scope.TenantID, WorkspaceID: scope.WorkspaceID,
				NodeID: runs[index].SubjectID, TaskRunID: runs[index].ID,
			})
			s.frameFailures.ReportVideoFrameStateFailure(
				reportCtx, runs[index].ID, frameTaskRunID,
				errors.New("associated first last frame task run is unavailable"),
			)
			continue
		}
		if !frameRun.Terminal() {
			runs[index].Status = domaintask.StatusRunning
...
type noopVideoFrameStateFailureReporter struct{}

func (noopVideoFrameStateFailureReporter) ReportVideoFrameStateFailure(context.Context, string, string, error) {
}
