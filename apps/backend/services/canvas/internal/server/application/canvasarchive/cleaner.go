...
func (cleaner *Cleaner) cleanupDueBatch(ctx context.Context) (int, int, error) {
	...
	for index := range items {
		...
		workCtx = logcontext.WithBusiness(workCtx, logcontext.Business{TaskRunID: item.TaskRunID})
		if cleaner.quota != nil {
			if _, markErr := cleaner.quota.MarkStorageReleasing(
				...
				failures = errors.Join(
					failures,
					platformtracecontext.WrapError(workCtx, cleaner.reschedule(workCtx, item, now, markErr)),
				)
				continue
			}
		}
		if deleteErr := cleaner.store.Delete(workCtx, item.OutputSHA256, item.TaskRunID); deleteErr != nil {
			failures = errors.Join(
				failures,
				platformtracecontext.WrapError(workCtx, cleaner.reschedule(workCtx, item, now, deleteErr)),
			)
			continue
		}
		if cleaner.quota != nil {
			...
			if releaseErr != nil {
				failures = errors.Join(
					failures,
					platformtracecontext.WrapError(workCtx, cleaner.reschedule(workCtx, item, now, releaseErr)),
				)
				continue
			}
		}
		...
		if completeErr != nil {
			failures = errors.Join(failures, platformtracecontext.WrapError(workCtx, completeErr))
			continue
		}
		if ok {
			completed++
