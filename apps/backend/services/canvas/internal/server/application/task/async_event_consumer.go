...
func (consumer *AsyncEventConsumer) ConsumeDue(ctx context.Context) (int, error) {
	...
	for index := range claims {
		...
		workCtx = logcontext.WithBusiness(workCtx, logcontext.Business{TaskRunID: event.TaskRunID})
		err = consumer.consume(workCtx, event, now)
		if err != nil {
			consumeErr = errors.Join(consumeErr, platformtracecontext.WrapError(
				workCtx,
				NewClaimError(event.TaskRunID, err),
			))
		}
	}
