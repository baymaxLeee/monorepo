...
func (s *Service) confirmResultCommit(ctx context.Context, taskRunID, artifactID string) (bool, error) {
	...
	runSucceeded := run.Status == domaintask.StatusSucceeded
	detailSucceeded := detail.Status == string(domaintask.StatusSucceeded)
	if runSucceeded && detailSucceeded && detail.AssetID != "" && s.assetManager != nil {
	...
	if !runSucceeded && !detailSucceeded && detail.AssetID == "" {
		return false, nil
	}
