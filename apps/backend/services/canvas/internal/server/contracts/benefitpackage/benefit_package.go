...
type GetPresetBenefitPackageRequest struct {
	Top *base.TopParam `json:"Top,omitempty"`
}

func NewGetPresetBenefitPackageRequest() *GetPresetBenefitPackageRequest {
	return &GetPresetBenefitPackageRequest{}
}

func (p *GetPresetBenefitPackageRequest) InitDefault() {
}

var GetPresetBenefitPackageRequest_Top_DEFAULT *base.TopParam

func (p *GetPresetBenefitPackageRequest) GetTop() (v *base.TopParam) {
	if !p.IsSetTop() {
		return GetPresetBenefitPackageRequest_Top_DEFAULT
	}
	return p.Top
}

func (p *GetPresetBenefitPackageRequest) IsSetTop() bool {
	return p.Top != nil
}

func (p *GetPresetBenefitPackageRequest) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("GetPresetBenefitPackageRequest(%+v)", *p)
}

type GetPresetBenefitPackageResponse struct {
	Package *BenefitPackage `json:"Package,omitempty"`
	// MaterialLimit 是 IAM 为当前租户配置的预置权益包素材数限额；Package 存在时返回。
	// optional 仅用于兼容滚动发布窗口内的旧 Server，调用方不得回退为固定值。
	MaterialLimit *int64 `json:"MaterialLimit,omitempty"`
}

func NewGetPresetBenefitPackageResponse() *GetPresetBenefitPackageResponse {
	return &GetPresetBenefitPackageResponse{}
}

func (p *GetPresetBenefitPackageResponse) InitDefault() {
}

var GetPresetBenefitPackageResponse_Package_DEFAULT *BenefitPackage

func (p *GetPresetBenefitPackageResponse) GetPackage() (v *BenefitPackage) {
	if !p.IsSetPackage() {
		return GetPresetBenefitPackageResponse_Package_DEFAULT
	}
	return p.Package
}

var GetPresetBenefitPackageResponse_MaterialLimit_DEFAULT int64

func (p *GetPresetBenefitPackageResponse) GetMaterialLimit() (v int64) {
	if !p.IsSetMaterialLimit() {
		return GetPresetBenefitPackageResponse_MaterialLimit_DEFAULT
	}
	return *p.MaterialLimit
}

func (p *GetPresetBenefitPackageResponse) IsSetPackage() bool {
	return p.Package != nil
}

func (p *GetPresetBenefitPackageResponse) IsSetMaterialLimit() bool {
	return p.MaterialLimit != nil
}

func (p *GetPresetBenefitPackageResponse) String() string {
	if p == nil {
		return "<nil>"
	}
	return fmt.Sprintf("GetPresetBenefitPackageResponse(%+v)", *p)
}
