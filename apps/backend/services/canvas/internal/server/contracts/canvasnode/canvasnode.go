...
import (
	...
	"database/sql/driver"
	"fmt"
	"github.com/example/monorepo/canvas/internal/server/contracts/asset"
	...
	"github.com/example/monorepo/canvas/internal/server/contracts/resource"
)
...
// CreateCanvasNodesRequest 创建持久化任务，并使用推理模型流式生成待确认候选分镜。
...
// GetCanvasNodeDraftsRequest 不接受客户端 event ID；TaskRunID 只选择任务，服务端按最新 DB 状态完整重放。
...
// CreateCanvasNodesResponse 定义该 Action 的 SSE 事件数据集合。
