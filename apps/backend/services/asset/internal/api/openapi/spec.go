...
func document() map[string]any {
	...
	return map[string]any{
		...
		"paths": map[string]any{
			"/assets": map[string]any{"post": map[string]any{"operationId": "assetUpload", "summary": "Stream one file into an immutable Asset revision", "parameters": []any{header("X-Auth-Tenant-ID"), header("X-Auth-Workspace-ID"), header("X-Auth-User-ID"), query("category")}, "requestBody": map[string]any{"required": true, "content": map[string]any{"multipart/form-data": map[string]any{"schema": map[string]any{"type": "object", "required": []string{"file"}, "properties": map[string]any{"file": map[string]any{"type": "string", "format": "binary"}}}}}}, "responses": map[string]any{"201": jsonResponse(map[string]any{"$ref": "#/components/schemas/UploadResult"}), "400": problemResponse(), "401": problemResponse()}}},
