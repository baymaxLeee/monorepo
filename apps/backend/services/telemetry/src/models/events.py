...
`id` is a server-side `bigint generated by default as identity` so the table has
a small, monotonically increasing key — the actual time-range queries hit the
`(app, *, ts_server)` secondary indexes.

`payload` is `jsonb`; SQLAlchemy serialises dict ↔ JSON for us.

`SessionRow` uses `(app, session_id)` as a composite primary key so that
ingestion can `INSERT ... ON CONFLICT DO UPDATE` and accumulate
