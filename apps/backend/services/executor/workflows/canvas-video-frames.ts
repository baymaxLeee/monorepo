...
export const canvasFramesInputSchema = z.object({ taskRunId: z.string().regex(/^[a-f0-9]{32}$/) });
type Input = z.infer<typeof canvasFramesInputSchema>;
async function extractFramesStep(input: Input) {
  "use step";
  ...
  try {
    return await client.executeFrames(input.taskRunId, cancellation.signal);
  } catch (error) {
...
export async function canvasFramesWorkflow(input: Input, executorTaskId: string) {
  "use workflow";
  await claimTaskStep(executorTaskId);
  return extractFramesStep(input);
}
