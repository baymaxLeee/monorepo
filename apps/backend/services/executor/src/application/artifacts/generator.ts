...
function blockInstructions(input: {
  ...
  return [
    ...
    "Inline JavaScript, canvas, controls, and event handlers are available for interactions, including custom reports, visualizations, and self-contained HTML5 games. Keep scripts self-contained, place them after the markup they initialize, and scope DOM queries to the current artifact block. Every static data-* selector used by a script must exist on its intended element. The compiler namespaces local ids and rewrites static '#id' selectors and getElementById('id') calls; prefer data-* selectors for dynamic code. Choose the rendering primitive from the information structure: conventional project or system architecture diagrams default to stable layered containers built with semantic HTML and CSS Grid/Flex, with inline SVG or Canvas for connectors and bespoke interaction. Use ECharts graph only for a relation-first network or dependency-topology view, never merely because the deliverable is called an architecture diagram. ECharts remains available for data charts and suitable interactive topology. When custom ECharts JavaScript is necessary, use the platform-provided window.echarts directly; the compiler delays ECharts-dependent scripts until the runtime is ready. Never load ECharts or any other external runtime yourself.",
