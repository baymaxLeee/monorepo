...
class BotService:
    def __init__(self, session: AsyncSession, user_id: str, redis: Redis | None = None) -> None:
        self._session = session
        self._user_id = user_id
        self._redis = redis

    async def list(self) -> list[Bot]:
        rows = await bot_crud.list_bots(self._session, self._user_id)
        return [to_schema(row) for row in rows]

    async def get(self, bot_id: str) -> Bot:
        row = await bot_crud.get_bot(self._session, bot_id, self._user_id)
        if row is None:
            raise NotFoundError(f"bot {bot_id} not found")
        return to_schema(row)

    async def create(self, name: str) -> Bot:
        row = await bot_crud.create_bot(self._session, name, self._user_id)
