...
class CanvasSettingsService:
    ...
    async def get(self) -> CanvasSettings:
        if not self.actor.org_id:
            raise ForbiddenError("active organization required")
        row = await repository.get(self.session, self.actor.org_id)
        if row is None:
    ...
    async def update(self, payload: UpdateCanvasSettings) -> CanvasSettings:
        if not self.actor.org_id or not self.actor.can_write_org_config:
        ...
        async with write_tx(self.session):
            for slot, kind in (("inference", "chat"), ("image", "image"), ("video", "video")):
                ...
                if selection is None:
                    continue
                provider = await providers.get_provider(self.session, selection.provider_id, self.actor.org_id)
                if provider is None or not provider.is_enabled or provider.provider_kind != kind:
                    raise RequestError(f"{slot} requires an enabled {kind} provider in this organization")
                if (
