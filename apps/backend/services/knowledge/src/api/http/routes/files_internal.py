...
async def read_file(
    ...
    return FileRead(
        **entry.model_dump(),
        ...
        next_offset=start + limit + 1 if start + limit < len(lines) else None,
        content=content,
    )
...
async def _change_set(session: DbSession, change_set_id: str, user_id: str, *, lock: bool = False) -> FileChangeSetRow:
    statement = select(FileChangeSetRow).where(
        FileChangeSetRow.id == change_set_id, FileChangeSetRow.user_id == user_id
    )
    if lock:
    ...
    async with write_tx(session):
        ...
        for entry in staged:
            ...
            if current_entry is None:
                ...
                current_entry.mime_type, current_entry.content, current_entry.sha256 = (
                    entry.mime_type,
                    entry.content,
                    entry.sha256,
                )
                current_entry.writable, current_entry.derived, current_entry.updated_at = (
                    entry.writable,
                    entry.derived,
                    now,
                )
