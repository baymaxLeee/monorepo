"""Durable artifact execution state and immutable revision graph."""
...
class ArtifactGenerationBlockRow(Base):
    __tablename__ = "artifact_generation_blocks"
    __table_args__ = (UniqueConstraint("generation_id", "block_id", name="ux_artifact_generation_block_id"),)
    ...
    status: Mapped[str] = mapped_column(String(24), nullable=False)
    version_id: Mapped[str | None] = mapped_column(String(32), index=True, nullable=True)
    error: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class ArtifactBlockVersionRow(Base):
    ...
    __table_args__ = (UniqueConstraint("document_id", "block_id", "object_sha256", name="ux_artifact_block_content"),)
...
class ArtifactRevisionBlockRow(Base):
    __tablename__ = "artifact_revision_blocks"
    ...
    version_id: Mapped[str] = mapped_column(String(32), index=True, nullable=False)
    position: Mapped[int] = mapped_column(Integer, nullable=False)
