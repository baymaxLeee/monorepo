...
import { cn, randomId } from "@repo/shared";
...
const filterFiles = (value: PromptInputValue, filesRef: MutableRefObject<Record<string, File>>) => {
  const tokenIds = new Set(value.tokens.map((token) => token.id));
  return Object.fromEntries(Object.entries(filesRef.current).filter(([id]) => tokenIds.has(id)));
};
...
const tokenSendState = (token: PromptInputToken): TokenSendState => {
  ...
  if ((token.kind === "file" || token.kind === "image") && typeof meta?.artifactId !== "string") {
    return "uploading";
  }
...
const PromptInputEditor = forwardRef<PromptInputRef, PromptInputProps>((props, ref) => {
  const {
    defaultValue,
  ...
  const extensions = useMemo<Extensions>(() => {
    const list: Extensions = [
      ...
      createPromptTokenExtension({
        renderToken: (token, context) => renderTokenRef.current?.(token, context),
        retryToken: (token) => {
          const file = filesRef.current[token.id];
          if (file) {
            onFilesAddedRef.current?.([{ token, file }]);
          }
        },
      }),
    ];
    if (mentionSource) {
      list.push(buildMentionExtension((query, signal) => mentionSourceRef.current?.(query, signal) ?? []));
    }
    if (slashCommands || onSkillsLoad || onSlashCommand) {
      list.push(
        buildSlashExtension({
          ...
          onCommand: (command, currentApi) => onSlashCommandRef.current?.(command, currentApi),
        }),
      );
    }
    return list;
  ...
  const emitChange = useCallback((currentEditor: Editor) => {
    ...
    const value = serializePromptInput(currentEditor.getJSON(), filesRef.current);
    ...
    setAttachmentsUploading(value.tokens.some((token) => tokenSendState(token) === "uploading"));
  ...
  insertFilesRef.current = (files: File[]) => {
    ...
    const existingCount = serializePromptInput(currentEditor.getJSON(), filesRef.current).tokens.length;
    ...
    for (const file of files.slice(0, remaining)) {
      ...
      if (file.size > maxFileSize) {
        onError?.(`${file.name} 超过 ${Math.round(maxFileSize / 1024 / 1024)} MB 限制。`);
        continue;
      }
    ...
    (tokenRef: string, patch: Partial<PromptInputToken>) => {
      ...
      const applied = currentEditor.commands.command(({ tr, dispatch }) => {
        ...
        doc.descendants((node, pos) => {
          if (updated || node.type.name !== "promptToken") {
            return;
          }
          const attrs = node.attrs as PromptInputToken;
          if (!tokenMatchesRef(attrs, tokenRef)) {
            return;
          }
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        ...
        handleKeyDown: (view, event) => {
          ...
          if (event.key === "Enter" && !event.shiftKey && !event.isComposing && !(event.metaKey || event.ctrlKey)) {
            event.preventDefault();
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!value.text.trim() && value.tokens.length === 0) {
      return;
    }
  ...
  return (
    <form className={cn("prompt-input", className)} style={style} onSubmit={handleSubmit}>
      <input
        ref={fileInputRef}
        type="file"
        multiple
      ...
      <div className="prompt-input-footer">
        ...
        <Button
          ...
          aria-label={loading ? "Stop" : attachmentsUploading ? "附件上传中" : "Submit prompt"}
