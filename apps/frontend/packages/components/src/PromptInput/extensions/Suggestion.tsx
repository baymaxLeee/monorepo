 ...
 */
export function createSuggestionExtension<T>(config: SuggestionExtensionConfig<T>) {
  const List = forwardRef<SuggestionListHandle, SuggestionProps<T, T>>(function PromptSuggestionList(props, ref) {
    ...
    addProseMirrorPlugins() {
      return [
        Suggestion<T, T>({
          ...
          command: ({ editor, range, props }) => config.onSelect(editor, range, props),
          render: () => {
            ...
            return {
              ...
              onKeyDown: (props) => renderer?.ref?.onKeyDown({ event: props.event }) ?? false,
              onExit: () => {
                unmount?.();
                renderer?.destroy();
                renderer = null;
