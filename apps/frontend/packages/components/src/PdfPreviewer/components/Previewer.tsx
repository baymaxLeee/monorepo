...
const isKeywordHighlight = (highlight: PdfHighlight): highlight is PdfHighlightKeyword =>
 ...
 */
const dropManagedKeys = <T extends object>(source: T | undefined, managed: ReadonlyArray<string>): Partial<T> => {
  if (!source) {
    return {};
  }
...
interface PageProxyLike {
  getViewport: (params: { scale: number; rotation?: number }) => PageViewportLike;
}
...
const getPageViewport = (page: unknown, params: { scale: number; rotation?: number }): PageViewportLike | null => {
  try {
 ...
 * - 退化矩形（< 0.5px）丢弃，避免脏数据
 */
export const computeSelectionRegions = (range: Range, root: HTMLElement): SelectionComputeResult | null => {
  ...
  if (regions.length === 0 || !anchorFound) {
    return null;
  }
...
const resolveBestFitScale = ({
  ...
  if (fitMode === "page-width") {
    ...
    nextScale = Math.min(availableW / baseViewport.width, availableH / baseViewport.height);
  }

  return clampScale(nextScale, minScale, maxScale);
};

const PdfPreviewerInner = forwardRef<PdfPreviewerRef, PdfPreviewerProps>((props, ref) => {
  const {
    file,
  ...
  const toolbarConfig = useMemo(() => resolveToolbarConfig(toolbar), [toolbar]);
  const toolbarVisible = useMemo(
    () => isToolbarVisible(toolbarConfig) || (toolbar !== false && Boolean(toolbarExtraRender)),
  ...
  const fitModeRef = useRef<FitMode | null>(typeof layout === "number" ? null : layout);
  const [scale, setScale] = useState<number | undefined>(typeof layout === "number" ? layout : undefined);
  const [rotation, setRotation] = useState<0 | 90 | 180 | 270>(0);
  const [loadStatus, setLoadStatus] = useState<"idle" | "loading" | "ready" | "error">("idle");
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [sidebarType, setSidebarType] = useState<PdfSidebarType>(defaultSidebar);
    ...
    return (props: { str: string }): string => {
      ...
      keywordHighlights.forEach((highlight) => {
        ...
        const pattern = (() => {
          ...
          return new RegExp(highlight.keyword.source, flags.includes("g") ? flags : `${flags}g`);
        })();
        const bg = highlight.color ?? "rgb(250 204 21 / 35%)";
        const styleAttr = ` style="background:${bg}"`;
        const cls = cn("rounded-sm", highlight.className);
        const dataId = highlight.id ? ` data-highlight-id="${highlight.id}"` : "";
        html = html.replace(
          pattern,
          (match) => `<mark ${KEYWORD_MARK_DATA_ATTR}="" class="${cls}"${styleAttr}${dataId}>${match}</mark>`,
        );
      });
      return html;
    };
  ...
  const composedCustomTextRenderer = useMemo(() => {
    ...
    if (!keywordTextRenderer && !userRenderer) {
      return undefined;
    }
  ...
  const scheduleFitScale = (fitMode: FitMode, targetPage?: unknown) => {
    ...
    autoScaleRafRef.current = window.requestAnimationFrame(() => {
      ...
      setScale((prev) => (prev !== undefined && Math.abs(prev - nextScale) < 1e-4 ? prev : nextScale));
    });
  };
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          visiblePageMapRef.current[pageNumber] = entry.isIntersecting ? entry.intersectionRect.height : 0;
        });
        syncCurrentPage();
      },
      {
  ...
  useEffect(() => {
    ...
    if (initialContinuousScrollDoneRef.current) {
      return;
    }
  ...
  const handleDocumentLoadSuccess = (pdf: LoadedFileLike) => {
    ...
    if (typeof layout === "number") {
      ...
      void pdf
        .getPage(next)
        .then((page) => {
          if (documentLoadTokenRef.current !== loadToken || pdfDocumentRef.current !== pdf) {
            return;
          }
  ...
  const handlePageRenderSuccess = (pageProxy: unknown) => {
    if (pendingScrollHighlightIdRef.current) {
      ...
      window.requestAnimationFrame(() => {
        const element = highlightLayerRef.current?.getHighlightElement(id) ?? null;
        if (element) {
          pendingScrollHighlightIdRef.current = null;
    ...
    userOnRenderSuccess?.(pageProxy as Parameters<NonNullable<typeof userOnRenderSuccess>>[0]);
  };
  ...
  const goToPage = (page: number) => {
    ...
    const next = Math.min(Math.max(1, Math.trunc(page)), numPages || Math.max(1, currentPage));
    window.requestAnimationFrame(() => {
      scrollToPage(next);
    });
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (file instanceof Blob) {
        const filename = file instanceof File && file.name ? file.name : DEFAULT_DOWNLOAD_FILENAME;
        triggerDownload(file, filename);
        return;
      }

      if (file instanceof ArrayBuffer) {
        triggerDownload(new Blob([file], { type: "application/pdf" }), DEFAULT_DOWNLOAD_FILENAME);
        return;
      }
      ...
      if (typeof file === "object" && "data" in file) {
        ...
        if (blobPart) {
          triggerDownload(new Blob([blobPart], { type: "application/pdf" }), DEFAULT_DOWNLOAD_FILENAME);
          return;
        }
      }
  ...
  const handleKeywordHighlightClick = (event: ReactMouseEvent<HTMLDivElement>) => {
    if (!onHighlightClick || keywordHighlights.length === 0) {
      return;
    }
    ...
    const mark = target.closest(`[${KEYWORD_MARK_DATA_ATTR}]`) as HTMLElement | null;
    if (!mark) {
      return;
    }
    const id = mark.getAttribute("data-highlight-id") || undefined;
    const index = keywordHighlights.findIndex((highlight) => (highlight.id ?? undefined) === id);
  ...
  const handleRegionHighlightClick = (payload: { highlight: PdfHighlight; index: number; event: MouseEvent }) => {
    if (!onHighlightClick) {
      return;
    }
  ...
  const scrollToHighlightById = (id: string) => {
    ...
    const existingElement = highlightLayerRef.current?.getHighlightElement(id);
    if (existingElement) {
      activateHighlightElement(existingElement);
      return;
    }

    if (!isRegionsHighlight(target)) {
      return;
    }
  ...
  useEffect(() => {
    if (autoScrollToFirstHighlight === false) {
      return;
    }
    if (loadStatus !== "ready") {
      return;
              ...
              {scale !== undefined ? (
                pageNumbers.map((pageNumber) => (
                  ...
                  >
                    ...
                    />
                    <HighlightLayer
                      ref={pageNumber === currentPage ? highlightLayerRef : undefined}
                      pageNumber={pageNumber}
                      highlights={regionHighlights}
                      onHighlightClick={onHighlightClick ? handleRegionHighlightClick : undefined}
                    />
                  </div>
                ))
              ) : (
