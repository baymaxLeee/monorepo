...
function parseQuestion(input: unknown): AskUserQuestion | null {
  ...
  if (typeof raw.id !== "string" || !raw.id.trim() || typeof raw.question !== "string" || !raw.question.trim()) {
    return null;
  }
  const choices = Array.isArray(raw.choices)
    ? raw.choices.flatMap((item) => {
        ...
        return typeof choice.label === "string" && typeof choice.value === "string"
  ...
  return {
    ...
    freeformLabel:
      typeof raw.freeform_label === "string" && raw.freeform_label.trim() ? raw.freeform_label.trim() : "其他",
  };
}
...
export function parseAskUserOutput(output: unknown): AskUserOutput | null {
  ...
  if (envelope.status !== "completed" || !envelope.data || typeof envelope.data !== "object") {
    return null;
  }
  ...
  const answers = raw.answers.flatMap((item) => {
    ...
    const values = Array.isArray(answer.values)
      ? answer.values.filter((value): value is string => typeof value === "string" && value.trim() !== "")
      : [];
    return typeof answer.id === "string" && values.length > 0 ? [{ id: answer.id, values }] : [];
  });
  return answers.length === raw.answers.length ? { answers } : null;
}

export function answerLabels(question: AskUserQuestion, values: string[]): string {
  const labels = new Map(question.choices.map((choice) => [choice.value, choice.label] as const));
