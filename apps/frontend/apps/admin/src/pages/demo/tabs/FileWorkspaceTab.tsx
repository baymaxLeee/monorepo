import { type FileChange, type FileNode, FileWorkspace } from "@repo/editors/file-workspace";
...
const demoFileTree: FileNode[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        type: "file",
        parent_id: "src",
        content: "你是客服质检助手。先确认用户诉求，再根据工单、订单与知识库给出可执行建议。",
      },
    ],
  },
  {
...
export function FileWorkspaceTab() {
  ...
  return (
    <DemoCard title="FileWorkspace" description="目录树、多标签和代码编辑器组合，内置项目文件数据" fill>
      ...
      <div className="shrink-0 rounded-md border bg-muted/40 p-3">
        <div className="mb-2 text-xs font-medium text-muted-foreground">最近一次变更</div>
        <pre className="max-h-32 overflow-auto text-xs leading-5">
          {lastFileChange ? JSON.stringify(lastFileChange, null, 2) : "尚未修改文件"}
        </pre>
      </div>
    </DemoCard>
  );
