...
Shell is the host application. It owns:
...
## Hard rules
- Shell MUST NOT contain business logic — that belongs in MFEs
- Shell route additions REQUIRE updating `src/registry.ts`
...
## When to extend the shell vs. extend an MFE
- Shell: authentication, layout chrome, route shell — anything that must be
