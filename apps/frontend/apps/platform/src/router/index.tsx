...
function relativeAppPath(app: AppEntry): string {
  ...
  if (!basePath.startsWith(PLATFORM_PREFIX)) {
    throw new Error(`App "${app.id}" base path must start with ${PLATFORM_PREFIX}`);
  }
...
function findAppForPath(apps: AppEntry[], path: string): AppEntry | undefined {
  return apps
    ...
    .sort((left, right) => normalizedBasePath(right).length - normalizedBasePath(left).length)[0];
}
...
function loadRemoteRoutes(app: AppEntry): Promise<RouteObject[]> {
  ...
  const promise = loadRemote<RemoteRoutesModule>(remoteModuleId(app)).then((module) => {
    if (!module || !Array.isArray(module.routes)) {
      throw new Error(`Remote "${app.remote_name}" did not expose a routes array at ${app.expose_key}`);
    }
...
async function discoverRemoteRoutes({ path, patch, signal }: PatchRoutesOnNavigationFunctionArgs): Promise<void> {
  if (!path.startsWith(PLATFORM_PREFIX)) {
    return;
  }

  if (!(await canDiscoverPlatformRoutes(signal))) {
    return;
  }

  const apps = await loadApps();
  const app = findAppForPath(apps, path);
  if (!app || patchedRemoteIds.has(app.id)) {
    return;
  }

  const children = await loadRemoteRoutes(app);
  if (patchedRemoteIds.has(app.id)) {
    return;
  }
