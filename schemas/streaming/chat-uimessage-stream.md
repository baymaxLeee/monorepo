...
- Known non-redundant overlaps to leave alone:
  ...
  - `generate_image` (media generation) returns its result **as tool output**,
    ...
    `{ status: "generating" }` then a terminal
    `{ status: "completed", images: [{ document_id, filename, media_type }] }`
    (or `{ ok: false, error }`). The frontend `ChatImageCard` renders the images
    inline from those `document_id`s (served via `/documents/:id/source`, the
    same route `file` parts use) — no `data-*` part is needed because the tool
    part already carries everything. Generated bytes live in Knowledge, never a
    temporary provider URL (ADR-0014).
